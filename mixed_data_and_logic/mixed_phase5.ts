...
function convertWeightUnits(weight: number, unit: string) {
  switch (unit) {
    case "oz":
      return weight * 16;
    case "kg":
...
function groceryInformation(infoType: string, groceryList: GroceryItem[]) {
  switch (infoType) {
    ...
    case "totalWeightInOz":
      return groceryList.reduce((acc, item) => {
        if (item.billingType === BillingType.LBS) {
          return acc + convertWeightUnits(item.quantity, "oz");
        }
        return acc;
      }, 0);
    case "totalWeightInKg":
      return groceryList.reduce((acc, item) => {
        if (item.billingType === BillingType.LBS) {
          return acc + convertWeightUnits(item.quantity, "kg");
        }
        return acc;
      }, 0);
    case "totalWeightInG":
      return groceryList.reduce((acc, item) => {
        if (item.billingType === BillingType.LBS) {
          return acc + convertWeightUnits(item.quantity, "g");
        }
        return acc;
      }, 0);
    case "totalEach":
